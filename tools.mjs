...
// NOTE: a backgrounded wait also does NOT wake an idle session (verified CC v2.1.214), so this
// clamp is about connection survival only — it is NOT a durable-listening mechanism. For real
// passive listening use a channels-enabled launch; otherwise an external re-invoker (ScheduleWakeup
// / cron -> check_messages). See docs/plans/wait-transport-cap-investigation.md.
...
export function registerTools(server, db, planChecker = null) {
  ...
  server.tool(
    "subscribe",
    "Register interest in a channel. Whether this gets you live delivery depends on how YOUR session was launched. Channels-enabled session (Claude Code started via cc-listen, or with --channels / --dangerously-load-development-channels naming this server): new messages ARE injected into your context live and will wake you when idle. Any OTHER session (the default): your client drops these notifications — subscribing alone is NOT listening, you will never see a message because of it, and your only options are a short (~2 min) foreground block in wait_for_reply, polling check_messages on a later turn, or an external re-invoker (ScheduleWakeup / cron) that calls check_messages on an interval. A BACKGROUNDED wait_for_reply does NOT wake an idle session, so it is not listening either. Only tell the user you are receiving live messages if you KNOW the session was launched channels-enabled; if unsure, assume it was not. Call 'register' first with the same instance_id.",
    {
  ...
  server.tool(
    "wait_for_reply",
    "Block inside this tool call, polling a channel until a new message arrives from another instance. Persistent by default — the call itself keeps polling across cycles until a message arrives, a 'done' signal is received, or the transport-safe ceiling is reached (~25 min); pass persistent: false for one-shot polling.\n\nTHIS IS A ~2-MINUTE FOREGROUND BLOCK, NOT BACKGROUND LISTENING (read carefully): Claude Code auto-backgrounds any MCP call past ~120s. A BACKGROUNDED wait_for_reply does NOT wake an idle session when a message arrives — verified 2026-07-18 on CC v2.1.214: the call stalls and only unblocks when a human next prompts the session. So it genuinely listens only for the ~2 min you are synchronously blocked; after that you are effectively deaf until re-invoked. Do NOT tell the user 'a background wait will wake me' — that is FALSE in a plain (non-channels) session. (Only sessions launched channels-enabled via cc-listen / --channels get real passive push that wakes an idle session.)\n\nTO KEEP LISTENING WITHOUT A CHANNELS-ENABLED SESSION: use an external re-invoker — schedule a wakeup (e.g. ScheduleWakeup) or cron that re-invokes you every few minutes to call check_messages — NOT a long wait_for_reply, which silently goes deaf at ~120s. max_wait_minutes is clamped to ~25 min (a single request can't outlive the server's ~30-min cap anyway); when it returns on the ceiling with no message, you are not passively listening — poll again on your next turn or schedule a re-invoker.\n\nROLE: pass role='parked' if you are a background/worker agent who wants to keep listening but must never pull an active coordinator out of ITS wait. Parked waiters still receive every message; they just don't count as a mutual-wait party. Default role='active'.\n\nONE WAIT PER CHANNEL: starting a new wait on a channel you're already waiting on supersedes the old one (the old call returns a clear 'superseded' result). No stacking.",
    {
    ...
    async ({ channel, after_id, instance_id, timeout_seconds, poll_interval_seconds, persistent, max_wait_minutes, role }, extra) => {
      ...
      // request cap, so promising more would be a lie. (Not a listening mechanism — a backgrounded
      // wait does not wake an idle session; use channels or an external re-invoker for that.)
      ...
      try {
        while (true) {
          ...
          if (Date.now() >= hardDeadline) {
            ...
            return {
              content: [{ type: "text", text: `⏳ This wait hit the ~${effectiveCeilingMin}-min transport-safe ceiling with no new message in #${normalized} (expected, not a failure). You are NOT passively listening now — a returned wait delivers nothing further, and a backgrounded wait_for_reply does not wake an idle session.\n\nTO KEEP LISTENING: schedule an external re-invoker (ScheduleWakeup / cron) that calls check_messages every few minutes, or just call check_messages yourself on your next turn — do NOT tell the user you're "still listening" in the meantime. If a peer might also be waiting, send a message to break the tie instead, or check list_instances. For real passive push that wakes an idle session, relaunch channels-enabled (cc-listen / --channels).` }],
            };
          }
...
- A BACKGROUNDED wait does NOT wake an idle session (verified 2026-07-18, CC v2.1.214): it stalls and only unblocks when a human next prompts the session. "A background wait will wake me" is FALSE in a plain session — do not say it. This is a harness limitation, not a server bug (delivery works; the harness just doesn't re-invoke an idle session on a backgrounded MCP call). To keep listening without a channels-enabled session, use an external re-invoker (ScheduleWakeup / cron) that calls \`check_messages\` on an interval; only a channels-enabled launch (cc-listen / --channels) gives real passive push
