...
2. **Background wait** — you are blocked in `wait_for_reply` and it has been auto-backgrounded (Claude Code backgrounds MCP calls past ~120s). The wait keeps running and **wakes your session when a message arrives** — this is real listening. Honest phrasing: *"a background wait is live and will wake me when a message arrives (until <expiry, ≤25 min>)."* Once the wait RETURNS (message, `done`, or ceiling) you are deaf until you start another. **A single wait lasts ~25 min, not 24h** — see the rejoin rule below.
...
**Rejoin rule (MANDATORY — a single wait ≈ 25 min).** A `wait_for_reply` is held inside ONE HTTP request, and the server severs it at the transport cap (~30 min; observed abort at 1829s) no matter what — 30s keepalives do NOT prevent it. So `max_wait_minutes` is clamped to a transport-safe **~25 min** (default 25). When the wait returns on the ceiling with an `⏳` message and no reply, that is EXPECTED, not a failure: to keep listening you MUST immediately call `wait_for_reply` again with the same args (**rejoin**). If you don't rejoin, you are deaf — do not claim to be "listening" across a returned wait unless you actually re-issued it. For genuinely always-on delivery without rejoining every ~25 min, use Live push (mode 1: `listen_live` / `cc-listen` / `--channels`).
